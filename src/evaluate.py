...
def evaluate(model, data, split, live, save_path):
    """
...
def main():
    ...
    train_file = os.path.join(sys.argv[2], "train_encoded.csv")
    test_file = os.path.join(sys.argv[2], "test_encoded.csv")
    ...
    with Live(EVAL_PATH) as live:
        ...
        save_importance_plot(live, model, train.columns[1:])
